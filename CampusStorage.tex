...
Shared data is used by multiple jobs.  In many cases, the job sandbox can be considered shared data.  Examples of shared data are:
\begin{itemize}
\item Input databases which will be queried by each job.  The queries may be unique per job, but the database job.  An example of this is BLAST \cite{altschul1990basic}.
\item 
\item Job executables and libraries.  Frequently the job executables are the same for a large number of jobs.  The arguments to the executables contextualize the jobs.

\end{itemize}

% May go into 
Many optimizations may be done to transfer shared data.  For example, people have used caching \cite{blumenfeld2008cms} the shared data per site.  Others have experimented using group transfer protocols such as bittorrent to distributed the shared data \cite{wei2005collaborative}.

% unique data
Data which is unique for each job is 
