...
Figure \ref{fig:boscojobsubmitflow} shows the job flow from the user to the Slurm scheduler on the remote cluster.  As you can see it goes through 6 daemons before reaching the cluster scheduler.  The SchedD accepts the user's jobs, acting as an agent which will attempt to execute the job.  The GridManager is spawned by the SchedD in order to service the Bosco Job.  The RemoteGahp (Grid Ascii Helper Protocol) starts the SSH tunnel between the user submit host and the cluster.  The Blahp (C portion) interprets the job requirements, and executes the Blahp (shell portion) which translates the requirements into the local scheduler language.  Finally, the Blahp submits the jobs to the Slurm Scheduler.
