...
\begin{enumerate}
	\item The Key-Value structure of ClassAds allow for attributes of the entities to be expressed in strings, values, and boolean expressions.
...
In order to illustrate a user's experience with the policy language, I will use the BLAST example that is used in the previous chapter's experiments.  In this example, we have two types of data:

\begin{description}
	\item[BLAST Database:] A shared public database which will be used by ever execution of the job.  The database is widely distributed, therefore there is no private data to hide.
...
Next, the BLAST executables are much smaller than the BLAST database, but they are used by every execution.  Therefore, they would benefit from caching and we will use the CacheD.  Since the replication shouldn't take long, there is no need to proactively replicate the executables.  But, there is nothing private in the BLAST executables, therefore they will be distributed over BitTorrent or Direct method.
...
Finally, the BLAST queries may be private.  The queries are stored in many very small files.  The queries may or may not be optimally transfered and managed by the CacheD.  If the BLAST jobs are short, then the submission node could constantly be transferring the query files to resources that need them.  If the number of running jobs is significant, then this could become a bottleneck.  Using the CacheD, the researcher could mitigate a lot of this transferring.  By grouping all the queries into a single cache, then transferring that cache for each job, any other jobs on that same node would request the file from the cache rather than the submission node.  If the jobs are short, then this cache would be used very often, at the cost of transferring all of the queries the first time.

Since the queries are private, we must transfer them with the Direct method so that the transfers are authenticated and encrypted.  Further, we must only replicate them to the nodes that actually request them, so as to not expose the queries on more nodes than necessary.
...
In this chapter, I have designed a policy language based on the HTCondor ClassAds that can be used for expressing policy in data distribution.  I have identified three interaction points for caching agents and designed the semantics for their interaction.
...
The policy language has been implemented in the CacheD, where it has been tested in Chapter \ref{chapter:campusdatadistribution}.  Further, we show the user experience of setting this policy language.
