...
In the previous chapter, Chapter \ref{chapter:campusdatadistribution}, I discussed the HTCondor CacheD.  In this section, I will discuss the policy framework that allows the CacheD to represent heterogeneous resources on campus or cyberinfrastructure resources.  The CacheD's policy framework inserts itself whenever it interacts with another CacheD.  This policy framework allows the CacheD to act as an independent agent within the distributed system.
...
I implement this policy language in the CacheD.  The policy language chosen for the CacheD is the HTCondor ClassAds \cite{raman1998matchmaking}.  ClassAds are an independent library developed by the HTCondor group that are used for communication between HTCondor components.  They provide all of the attributes described above.

\begin{enumerate}
	\item The Key-Value structure of ClassAds allow for attributes of the entities to be expressed in strings, values, lists of string, or boolean expressions.
...
Each cache has a single origin CacheD.  This CacheD is usually the same CacheD for which the user initially uploaded the cache.  This origin CacheD has the option to proactively replicate the cache without it being requested by jobs.  The data transfer can occur while another job is currently being run.
...
Next, the BLAST executables are much smaller than the BLAST database, but they are used by every execution.  Therefore, they would benefit from caching and I will use the CacheD.  Since the replication shouldn't take long, there is no need to proactively replicate the executables.  But, there is nothing private in the BLAST executables, therefore they will be distributed over BitTorrent or Direct method.
